...
public class SpeedyExceptionHandler implements InitializingLogBean {
    ...
    public ResponseResult<Void> dealArithmeticException(ArithmeticException e) {
        return dealFlow(e, () -> DefaultNormalResultStatus.FAILED.response("发生数学异常：" + e.getMessage()));
    }
    ...
    public ResponseResult<Void> dealHttpMessageNotReadableException(HttpMessageNotReadableException e) {
        return dealFlow(e, () -> DefaultNormalResultStatus.FAILED.response("请求参数格式错误：" + e.getMessage()));
    }

    @ExceptionHandler(ServletRequestBindingException.class)
    public ResponseResult<Void> dealMissingServletRequestParameterException(ServletRequestBindingException e) {
        return dealFlow(e, () -> DefaultNormalResultStatus.FAILED.response("请求参数错误：" + e.getMessage()));
    }

    @ExceptionHandler(BindException.class)
    public ResponseResult<Void> dealBindException(BindException e) {
        return dealValid(e, e.getBindingResult());
    }

    // 这个异常发生在基于@Valid注解的请求参数校验失败
    @ExceptionHandler(MethodArgumentNotValidException.class)
    public ResponseResult<Void> dealMethodArgumentNotValidException(MethodArgumentNotValidException e) {
        return dealValid(e, e.getBindingResult());
    }

    private ResponseResult<Void> dealValid(Exception e, BindingResult bindingResult) {
        Map<String, List<String>> map = new HashMap<>(bindingResult.getErrorCount());
        bindingResult.getFieldErrors().forEach(fieldError -> {
            String field = fieldError.getField();
            List<String> messages = bindingResult.getFieldErrors(field)
                    .stream()
        ...
        return dealFlow(e, () -> DefaultNormalResultStatus.FAILED.response("请求参数校验失败：" + map));
    }
}
